...
Cell pred; {
    ...
    checkKind(line,arg(pred),NIL,predicate,class(fun(pred)).sig,0);
}
...
Cell c; {
	...
	switch (whatIs(tycon(c).what)) {
	    case DATATYPE :
	    case DERIVE   : bindTv(typeOff+tycon(c).arity,STAR,0);
	}
	tycon(c).kind = mkInt(beta);
    }
    else
	class(c).sig = mkInt(newKindvars(1));
}
...
Cell c; {				/* is well-kinded		   */
	...
	switch (whatIs(tycon(c).what)) {
	    case DATATYPE    :
	    case DERIVE      : {   List cs = tycon(c).defn;
				   if (whatIs(cs)==QUAL) {
	...
	for (; nonNull(scs); scs=tl(scs))
	    if (!kunify(class(hd(scs)).sig,0,class(c).sig,0)) {
		ERROR(class(c).line)
		    "Kind of class \"%s\" does not match superclass \"%s\"",
		    textToStr(class(c).text), textToStr(class(hd(scs)).text)
		EEND;
	    }
...
Cell c; {				/* given tycon/class		   */
	...
	class(c).sig = copyKindvar(intOf(class(c).sig));
#ifdef DEBUG_KINDS
	printf("%s :: ",textToStr(class(c).text));
	printKind(stdout,class(c).sig);
	putchar('\n');
#endif
    }
}
