 ...
 * hugs.c:      Copyright (c) Mark P Jones 1991-1994.   All rights reserved.
...
#ifdef LESS_OBNOXIOUS
    printf("Hugs, The Haskell User's Gofer System Version 1.01 (Derived from Gofer 2.30b)\n");
    printf("Copyright (c) Mark P Jones, 1994.\n\n");
#else
    printf("     ___    ___   ___    ___   __________   __________\n");
    printf("    /  /   /  /  /  /   /  /  /  _______/  /  _______/  Version 1.01\n");
    printf("   /  /___/  /  /  /   /  /  /  / _____   /  /______\n");
    printf("  /  ____   /  /  /   /  /  /  / /_   /  /______   /    Copyright (c)\n");
    printf(" /  /   /  /  /  /___/  /  /  /___/  /  _______/  /     Mark P Jones\n");
    printf("/__/   /__/  /_________/  /_________/  /_________/      1994\n\n");
    printf(" The Haskell User's Gofer System.   Derived from Gofer 2.30b.\n");
    printf(" compiled for OS/2 Warp, using GCC+emx, by Michael Zimmermann 15.04.96\n\n");
#endif
    fflush(stdout);
...
String argv[]; {
    ...
    numScripts    = 0;
    namesUpto     = 1;
    scriptName[0] = strCopy(fromEnv("HUGS",STD_PRELUDE));
    prompt        = strCopy("?");
    repeatStr     = strCopy("$$");
...
Text t; {
    ...
    if (nonNull(cl)) {                  /* as a class                      */
        ...
        if (class(cl).sig==STAR)
            printf("-- type class");
        else {
            printf("-- constructor class");
            if (kindExpert) {
                printf(" with instances of kind ");
                printKind(stdout,class(cl).sig);
            }
        }
        printf("\nclass ");
        if (nonNull(class(cl).supers)) {
            List cs = class(cl).supers;
            if (nonNull(tl(cs))) {
                putchar('(');
                for (;;) {
                    printf("%s a",textToStr(class(hd(cs)).text));
                    if (nonNull(cs=tl(cs)))
                        printf(", ");
                    else
                        break;
        ...
        printf("%s a",textToStr(class(cl).text));
