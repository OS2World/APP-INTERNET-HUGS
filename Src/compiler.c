...
static Cell local matchNum		Args((Int,List,List,Cell));
...
#define VARDISCR	      0		/* variable (or wildcard)	   */
#define CONDISCR	      1		/* Constructor			   */
#define NUMDISCR	      2		/* Numeric pattern		   */

#define isConPat(discr)       (discrKind(discr)==CONDISCR)
#define isVarPat(discr)       (discrKind(discr)==VARDISCR)
#define isNumPat(discr)       (discrKind(discr)==NUMDISCR)
...
List us; {			       /* given by us.	co is the current  */
	...
	switch (discrKind(discr=hdDiscr(sws))) {
	    ...
	    case NUMDISCR : return matchNum(co,sws,us,discr);
	}
    }
...
List sws; {			       /* in first branch of a [Switch].   */
    Cell pat = hd(fst3(hd(sws)));
    Cell h   = getHead(pat);
    switch (whatIs(h)) {
#if NPLUSK
	case ADDPAT : return fun(pat);
#endif
...
Cell e; {
	...
	case AP	       : return NUMDISCR;  /* must be a literal or (n+k)   */
    }
    internal("discrKind");
    return 0;/*NOTREACHED*/
}
...
#if NPLUSK
	case AP	       : return (whatIs(fun(e))==ADDPAT) ? 1 : 0;
#else
...
/* --------------------------------------------------------------------------
 * Match on numeric literals or (n+k) patterns:
 * ------------------------------------------------------------------------*/

static Cell local matchNum(co,sws,us,discr) /* match against numeric vals  */
Int  co;
List sws;
List us;
Cell discr; {
    Cell d    = pmcTerm(co,snd3(hd(sws)),discr);
    Int  da   = discrArity(d);		/* 0 for literal, 1 for (n+k)	   */
    List nsws = NIL;
    Cell s;

    do {
        List ps       = switchPats(s=hd(sws));
	switchPats(s) = (da==0) ? tl(ps) : cons(arg(hd(ps)),tl(ps));
        nsws	      = cons(s,nsws);
	sws	      = tl(sws);
     } while (nonNull(sws) && isNumPat(discr=hdDiscr(sws))
			   && eqNumDiscr(d,discr));
     s = ap(NUMCASE,
	    triple(hd(us),
		   d,
